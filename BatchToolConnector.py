###############################################################################
##                FUSION:Connect Single Node to Many                         ##
##                                                                           ##
## This is a script that is usful when you have a node you need to connect   ## 
## to many. It works baised off position, so you need to have the conecting  ##
## node the left most. This scripts finds every possable connection and guess##
## what you want to connect, it is not always right and sometimes some       ##
## outputs can not connect to an input. To use select all the nodes you want ##
## to connect and it will assume your left most node is the one you want to  ##
## connect to many, then select the attrubtes you want to connect and hit ok ##
##                                                                           ##
## Created by Andrez Aguayo                                                  ##
## 3/13/15                                                                   ##
## Email: andrez@aguayo.me                                                   ##
## Please feel free to contact me with any questions, comments, suggestions  ##                           
## and bugs                                                                  ##
###############################################################################
...
def make_list_to_fusion_style_dictionary(theList):
	...
	fusion_style_dictionary = dict(zip(nuberedList,theList))
...
def make_that_connection(conecter, conecterOutput, conecty, conectyInput):	
	...
	for object in OPL.values():
		if object.Name ==  conecterOutput: 
			theOutput = object
		
	for objects in IPL.values():
...
def make_dialog(leftMost, outputlist, toolList, listOfListOfInputs):
	...
	dialog = {}
	dialog[1.0] = {1: " ", 2: "Text","Default":"",
				   "ReadOnly":True,"Width":2}
	dialog[2.0] = {1: "Output", 2: "Dropdown", 
                   "Name": leftMost.GetAttrs()['TOOLS_RegID'] +" Output:",
				   "Options": outPutDropdown, "Width":.5}
	dialog[3.0] = {1: "Input", 2: "Dropdown", 
                   "Name": toolList[0].GetAttrs()['TOOLS_RegID'] + " Input:", 
				   "Options": inputDropdown, "Width":.5}
	dialog[4.0] = {1: "OutPutOverride", 2: "Text","Name":"Output Override:",
				   "Default":"", "Width":.5}
	dialog[5.0] = {1: "InputOverride", 2: "Text","Name":"Input Override:", 
				  "Default":"", "Width":.5}
...
def do_it():
	...
	if leftMost != None:
		...
		if toolList != None:
			...
			## Make UI
			dialog, outPutDropdown, inputDropdown = make_dialog(leftMost, 
                                                                outputlist, 
                                                                toolList, 
                                                                listOfListOfInputs)
			...
			if userInfo != None:
				conecterOutput, conectyInput = process_userInfo(userInfo, outPutDropdown, 
                                                                inputDropdown)
					
				## OK LETS MAKE THE CONNECTIONS! 
				for tool in toolList:
					myConBoolean = make_that_connection(leftMost, conecterOutput, tool, conectyInput)
					if myConBoolean == False:
						print "Was not able to connect {0}.{1} with {2}.{3}".format(leftMost.Name, 
                                                                                    conecterOutput, 
                                                                                    tool.Name,
                                                                                    conectyInput) 		
			...
			print "Only ONE object is selected, you need at least two nodes to make a connection" 
